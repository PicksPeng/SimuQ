...
class BraketIonQCircuit(IonQCircuit):
    def __init__(self, num_qubits):
        ...
        self._braket_circuit = Circuit()

    def gpi(self, q, phi):
        self._braket_circuit.gpi(q, to_turns(phi + self._accum_phases[q]))

    def gpi2(self, q, phi):
        self._braket_circuit.gpi2(q, to_turns(phi + self._accum_phases[q]))

    def ms_quarter(self, q0, q1, phi0, phi1, theta):
        self._braket_circuit.ms(
            q0,
            q1,
    ...
    def optimize(self):
        ...
        """
        new_circ = BraketIonQCircuit(self._braket_circuit.qubit_count)

        n = len(self._accum_phases)

        unitaries = [np.eye(2)] * n

        for instruction in self._braket_circuit.instructions:
        ...
        new_circ._braket_circuit = Circuit().add_verbatim_box(new_circ._braket_circuit)
        return new_circ
